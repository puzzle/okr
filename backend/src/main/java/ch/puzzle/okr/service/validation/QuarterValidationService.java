...
public class QuarterValidationService extends
    ...
    public static void throwExceptionWhenStartEndDateQuarterIsNull(Quarter model) {
        if (!model.getLabel().equals(BACK_LOG_QUARTER_LABEL)) {
            if (model.getStartDate() == null) {
                throw new OkrResponseStatusException(HttpStatus.BAD_REQUEST, ErrorKey.ATTRIBUTE_NULL, List.of(
                                                                                                              "StartDate",
                                                                                                              model.getLabel()));
            } else if (model.getEndDate() == null) {
                throw new OkrResponseStatusException(HttpStatus.BAD_REQUEST, ErrorKey.ATTRIBUTE_NULL, List.of("EndDate",
                                                                                                              model.getLabel()));
            }
    ...
    public void validateOnGeneration(Quarter quarter) {
        if (quarter.getStartDate() == null) {
            throw new OkrResponseStatusException(HttpStatus.BAD_REQUEST, ErrorKey.ATTRIBUTE_NULL, List.of("StartDate",
                                                                                                          quarter.getLabel()));
