...
public record AuthorizationUser(User user) {
    public List<Long> extractTeamIds() {
        return this.user.getUserTeamList().stream().map(UserTeam::getId).toList();
    }

    public List<Long> extractTeamIdsWithAdminRole() {
        return this.user.getUserTeamList().stream().filter(UserTeam::isTeamAdmin).map(UserTeam::getId).toList();
    }
}
