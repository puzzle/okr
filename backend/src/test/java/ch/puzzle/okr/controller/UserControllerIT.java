...
class UserControllerIT {
    ...
    void shouldReturnCurrentUser() throws Exception {
        ...
        mvc.perform(get("/api/v1/users/current").contentType(MediaType.APPLICATION_JSON))
                .andExpect(MockMvcResultMatchers.status().isOk()).andExpect(jsonPath("$", Matchers.aMapWithSize(7)))
                .andExpect(jsonPath("$.id", Is.is(2))).andExpect(jsonPath("$.version", Is.is(3)))
                .andExpect(jsonPath("$.firstname", Is.is(FIRSTNAME_1)))
                .andExpect(jsonPath("$.lastname", Is.is(LASTNAME_1))).andExpect(jsonPath("$.email", Is.is(EMAIL_1)))
    ...
    void shouldCreateUsers() throws Exception {
        ...
        mvc.perform(post("/api/v1/users/createall").content(SUCCESSFUL_UPDATE_BODY)
                ...
                .andExpect(jsonPath("$[0].id", Is.is(2))).andExpect(jsonPath("$[0].version", Is.is(3)))
