...
describe("CustomizationService",
  () => {
    ...
    beforeEach(() => {
        ...
        getElementById: (id: string) => {
          return {
            setAttribute: function () {
              callRecorder.add(`${id}-setAttribute`,
                arguments);
            }
        ...
        querySelector: (selector: string) => {
          return {
            set innerHTML (value: string) {
              callRecorder.add(`${selector}.innerHTML`,
                arguments);
            },
            get style () {
              return {
                setProperty: function () {
                  callRecorder.add(`${selector}.style.setProperty`,
                    arguments);
                },
                removeProperty: function () {
                  callRecorder.add(`${selector}.style.removeProperty`,
                    arguments);
                }
              };
            }
      ...
      service = new CustomizationService(configServiceMock,
        documentMock);
    });
    ...
    it("should call correct apis when config is ready",
      () => {
        ...
        expect(callRecorder.getCallByIdx("title.innerHTML",
          0)[0])
          .toBe("title");
        expect(callRecorder.getCallByIdx("favicon-setAttribute",
          0)[0])
          .toBe("href");
        expect(callRecorder.getCallByIdx("favicon-setAttribute",
          0)[1])
          .toBe("favicon");
        expect(callRecorder.getCallByIdx("html.style.setProperty",
          0)[0])
          .toBe("--cssVar1");
        expect(callRecorder.getCallByIdx("html.style.setProperty",
          0)[1])
