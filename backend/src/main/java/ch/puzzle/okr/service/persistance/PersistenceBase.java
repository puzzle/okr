...
public abstract class PersistenceBase<Model, Id> {
    ...
    public Model findById(Id id) throws ResponseStatusException {
        return repository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, String.format("%s with id %s not found", getModelName(), id)));
    }
    ...
    private List<Model> iteratorToList(Iterable<Model> iterable) {
        return StreamSupport.stream(Spliterators.spliteratorUnknownSize(iterable.iterator(), Spliterator.ORDERED), false).toList();
    }
}
