...
describe('SearchTeamManagementComponent', () => {
  ...
  it('should generate html values correctly', () => {
    ...
    expect(filteredUsers.map(getHTMLValues)).toEqual([
      '<strong>Ruedi</strong> Peters (rpeter@gmail.com)'
    ]);

    expect(filteredTeams.map(getHTMLValues)).toEqual([
      'Team <strong>Ruedi</strong> - Noname',
    ]);
  });
