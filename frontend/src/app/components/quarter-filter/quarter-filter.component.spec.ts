...
describe("QuarterFilterComponent",
  () => {
    ...
    it("should set correct default quarter if no route param is defined",
      async () => {
        jest.spyOn(component,
          "changeDisplayedQuarter");
        jest.spyOn(quarters[2] as any,
          "isCurrent")
    ...
    it("should set default quarter if quarter id in route params does not exist",
      async () => {
        jest.spyOn(component,
          "changeDisplayedQuarter");
        const quarterSelect = await loader.getHarness(MatSelectHarness);
