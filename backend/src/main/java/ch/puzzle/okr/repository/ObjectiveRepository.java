...
public interface ObjectiveRepository extends CrudRepository<Objective, Long> {
    ...
    @Query(value = "select coalesce(AVG((CAST(coalesce(measure_value, 0) as decimal)/CAST(target_value as decimal))*100), 0) from\n"
            ...
            + "      from key_result LEFT JOIN measure m on key_result.id = m.key_result_id) as sub where objective_id = :objective_id group by id) as sub2", nativeQuery = true)
