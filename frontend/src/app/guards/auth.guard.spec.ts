...
describe("authGuard",
  () => {
    ...
    it("should not call initCodeFlow if token is valid and call router if state param exist",
      async () => {
        jest.spyOn(oAuthMock,
          "hasValidIdToken")
          .mockReturnValue(true);
        route.queryParamMap.set("state",
          1234);
    ...
    it("should not call router if state param does not exist",
      async () => {
        jest.spyOn(oAuthMock,
          "hasValidIdToken")
          .mockReturnValue(true);
    ...
    it("should call initCodeFlow if token is invalid",
      async () => {
        jest.spyOn(oAuthMock,
          "hasValidIdToken")
    ...
    async function runAuthGuardWithContext (authGuard: any): Promise<boolean> {
      return await TestBed.runInInjectionContext(() => authGuard(route,
        null));
    }
  });
