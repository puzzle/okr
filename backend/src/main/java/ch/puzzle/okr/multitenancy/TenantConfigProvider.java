...
public class TenantConfigProvider implements TenantConfigProviderInterface {
    ...
    public TenantConfigProvider(final @Value("${okr.tenant-ids}") String[] tenantIds, Environment env) {
        ...
        for (String tenantId : tenantIds) {
            ...
            tenantConfigs.put(tenantId, createTenantConfig(c.jwkSetUri(), c.frontendClientIssuerUrl(), c.frontendClientId(), tenantId));
        }
    }

    private OauthConfig readOauthConfig(String tenantId) {
        return new OauthConfig(env.getProperty(MessageFormat.format("okr.tenants.{0}.security.oauth2.resourceserver.jwt.jwk-set-uri", tenantId)), env.getProperty(MessageFormat.format("okr.tenants.{0}.security.oauth2.frontend.issuer-url", tenantId)), env.getProperty(MessageFormat.format("okr.tenants.{0}.security.oauth2.frontend.client-id", tenantId)));
    }
    ...
    private TenantConfig createTenantConfig(String jwkSetUriTemplate, String frontendClientIssuerUrl,
        ...
        return new TenantConfig(tenantId, getOkrChampionEmailsFromTenant(tenantId), jwkSetUriTemplate, frontendClientIssuerUrl, frontendClientId, this.readDataSourceConfig(tenantId));
    }
    ...
    private DataSourceConfig readDataSourceConfig(String tenantId) {
        return new DataSourceConfig(env.getProperty("okr.datasource.driver-class-name"), env.getProperty(MessageFormat.format("okr.tenants.{0}.datasource.url", tenantId)), env.getProperty(MessageFormat.format("okr.tenants.{0}.datasource.username", tenantId)), env.getProperty(MessageFormat.format("okr.tenants.{0}.datasource.password", tenantId)), env.getProperty(MessageFormat.format("okr.tenants.{0}.datasource.schema", tenantId)));
    }
