  ...
  () => {
    ...
    it.each([
      ...
      [3,
        ...
        1],
      true]
    ])("should give correct output for deep equal",
      (arr1: number[], arr2: number[], output: boolean) => {
        expect(areEqual(arr1,
          arr2))
          .toBe(output);
        expect(areEqual(arr2,
          arr1))
          .toBe(output);
      });
    ...
    it.each([
      ...
      [[],
        1,
        [1]]
    ])("should give correct output for getValueFromQuery",
      (value: any, fallback: number | undefined, arr2: number[]) => {
        expect(getValueFromQuery(value,
          fallback))
          .toStrictEqual(arr2);
      });
    ...
    it.each([
      ...
      [{ v: undefined,
        v2: 1 },
      { v2: 1 }]
    ])("should give correct output for optionalValue",
      (value: Record<string, any>, expected: Record<string, any>) => {
    ...
    it.each([[" test ",
      "test"],
    ["ffF",
      "fff"]])("test sanitize function",
      (str: any, expected: string) => {
        expect(sanitize(str))
          .toBe(expected);
      });
    ...
    test("should return correct class",
      () => {
        ...
        // False check
        expect(formInputCheck(testForm,
          "code"))
