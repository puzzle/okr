...
import { CheckInMinMetric } from '../shared/types/model/CheckInMin';
...
describe('CheckInService', () => {
  ...
  it('should map correctly', () => {
    service.getAllCheckInOfKeyResult(keyResultMetricWithIdEight.id).subscribe((checkIns) => {
      ...
      expect((checkIns[0] as CheckInMinMetric).value).toBe(firstCheckIn.value);

      expect(checkIns[1].confidence).toBe(secondCheckIn.confidence);
      expect((checkIns[1] as CheckInMinMetric).value).toBe(secondCheckIn.value);
    });
  });
});
