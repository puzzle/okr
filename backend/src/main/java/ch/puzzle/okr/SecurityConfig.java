...
public class SecurityConfig {
    ...
    private HttpSecurity setHeaders(HttpSecurity http) throws Exception {
        http.headers().contentSecurityPolicy("default-src 'self';" + "script-src 'self' 'unsafe-inline';"
                ...
                + "        base-uri 'self';"
                + "        connect-src 'self' https://sso.puzzle.ch http://lcoalhost:8544; https://sso.puzzle.ch/auth/realms/pitc/.well-known/openid-configuration; https://idp-mock-okr.ocp-internal.cloudscale.puzzle.ch;"
