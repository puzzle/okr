...
class QuarterPersistenceServiceIT {
    ...
    void shouldReturnCurrentQuarter() {
        ...
        assertTrue(LocalDate.now().isEqual(quarter.getStartDate()) || LocalDate.now().isAfter(quarter.getStartDate()));

        assertTrue(LocalDate.now().isEqual(quarter.getEndDate()) || LocalDate.now().isBefore(quarter.getEndDate()));
