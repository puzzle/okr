...
Cypress.Commands.add("loginAsUser",
  (user: any) => {
    loginWithCredentials(user.username,
      user.password);
    overviewIsLoaded();
  });

Cypress.Commands.add("getByTestId",
  (testId: string, text?: string): Chainable => {
...
Cypress.Commands.add("pressUntilContains",
  (text: string, key: keyof typeof keyCodeDefinitions) => {
    pressUntilContains(text,
      key);
  });
...
Cypress.Commands.add("tabForwardUntil",
  (selector: string, limit?: number) => {
    doUntilSelector(selector,
      cy.tabForward,
      limit);
  });

Cypress.Commands.add("tabBackwardUntil",
  (selector: string, limit?: number) => {
    doUntilSelector(selector,
      cy.tabBackward,
      limit);
...
Cypress.Commands.add("getZone",
  (zone: string, onOverview: boolean) => {
    return (onOverview ? cy.focused() : cy.getByTestId("side-panel")).findByTestId(zone);
  });

Cypress.Commands.add("validateScoring",
  (isOverview: boolean, percentage: number) => {
    validateScoring(isOverview,
      percentage);
  });
