...
describe("ConfidenceComponent",
  () => {
    ...
    it.each([[{ confidence: 8 } as CheckInMin,
      "8"],
    [null,
      "5"]])("should set confidence of component with right value",
      async (checkIn: CheckInMin | null, expected: string) => {
        component.checkIn = checkIn!;
        component.ngOnChanges({
          checkIn: new SimpleChange(null,
            component.checkIn,
            true)
        });
    ...
    it.each([[true],
      [false]])("should show slider on based on input var",
      async (editable) => {
