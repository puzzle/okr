...
public interface MeasureRepository extends CrudRepository<Measure, Long> {
    ...
    @Query(value = "select * from measure as ms where ms.created_on in (select max(m.created_on) from measure as m where m.key_result_id = :key_result_id) and ms.key_result_id = :key_result_id", nativeQuery = true)
