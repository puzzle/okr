...
describe("ErrorInterceptor", () => {
  ...
  it.each([
    ...
    "handleSuccessToaster should show toaster ", (url: string, code: number, method: HttpType, key: string, toasterType: ToasterType, message: string) => {
      ...
      jest.spyOn(interceptor, "getSuccessMessageKey")
        .mockReturnValue({ key: key, toasterType: toasterType });
  ...
  it.each([
    ...
    "checkIfSuccessToasterIsShown should work as intended", (currentURL: string, requestURL: string, result: boolean) => {
  ...
  it("should return custom success message on objective creation in backlog", () => {
    ...
    jest.spyOn(toaster, "showCustomToaster");
    jest.spyOn(interceptor, "getSuccessMessageKey")
      .mockReturnValue({ key: "OBJECTIVE.POST", toasterType: undefined });
    ...
    expect(toaster.showCustomToaster)
      .toBeCalledWith(
        "Das Objective wurde als Draft im Backlog gespeichert.", undefined,
      );
  });
});
