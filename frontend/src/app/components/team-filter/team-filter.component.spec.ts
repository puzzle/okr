...
describe("TeamFilterComponent",
  () => {
    ...
    it("should select all chips per default",
      waitForAsync(async () => {
        jest.spyOn(component.teams$,
          "next");
        jest.spyOn(component,
    ...
    it("should select the right chips",
      waitForAsync(async () => {
    ...
    it("activeTeams array should contain every team when all teams are shown",
      waitForAsync(async () => {
        const teamIds = teamList.map((e) => e.id);
        jest.spyOn(component.teams$,
          "next");
    ...
    it("change filter params and reload",
      fakeAsync(async () => {
        ...
        jest.spyOn(component,
          "changeTeamFilterParams");
        jest.spyOn(refreshDataServiceMock,
          "markDataRefresh");
    ...
    it.each([
      ...
      (activeTeams: number[], selected: number, expected: number[]) => {
    ...
    it.each([
      ...
      (activeTeams: number[], expected: boolean) => {
        component.activeTeams = activeTeams;
        expect(component.areAllTeamsShown())
          .toBe(expected);
      });
    ...
    it.each([
      ...
      (currentTeams: number[], expectedTeams: number[]) => {
    ...
    it("should use teams of user if no known teams are in url",
      async () => {
        const teamIds = [654,
          478];
        jest.spyOn(component.teams$,
    ...
    it("should use teams of user if no teams are in url",
      async () => {
        jest.spyOn(component.teams$,
          "next");
        jest.spyOn(component,
    ...
    "1,2,3"],
    [[],
      null]])("changeTeamFilterParams",
      async (currentTeams: number[], routingTeams: string | null) => {
        component.activeTeams = currentTeams;
    ...
    it("should filter teams by toggled priority and then by name",
      async () => {
        const teams = [
          { id: 1,
            version: 0,
